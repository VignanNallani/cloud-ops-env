...
API_BASE_URL = os.getenv("API_BASE_URL", "https://generativelanguage.googleapis.com/v1beta/openai/") [cite: 42]
MODEL_NAME = os.getenv("MODEL_NAME", "gemini-2.0-flash") [cite: 43]
HF_TOKEN = os.getenv("HF_TOKEN") [cite: 44]
...
async def run_logic(base_url: str):
    import websockets
    # [START] tag with all mandatory fields [cite: 21, 33]
    ...
    try:
        async with websockets.connect(ws_url, timeout=10) as ws:
            ...
            for t in range(20):
                ...
                # [STEP] tag with 2-decimal rewards and lowercase booleans [cite: 22, 28, 29]
        ...
        # [END] tag with comma-separated rewards [cite: 23, 37]
